...
// 탑바 2차 메뉴 나오는 함수
$('.header > .top-bar > .inner > .main-menu-box > ul > li' ).mouseenter(function () {
    var $header = $(this).closest('.header');

    var $this = $(this);
    var index = $this.index();
    var $thisContents =  $header.find(' > .contents > .inner > div').eq(index);
    var $currentContents = $thisContents.siblings('.active');

    console.log(index);

    $header.addClass('active');
    $header.find(' > .contents').addClass('active');
    $currentContents.removeClass('active');
    $thisContents.addClass('active');
...
$('.header' ).mouseleave(function (){
    ...
    $(this).find(' > .contents > .inner > div.active').removeClass('active');
...
function SliderK__initSideBtns($slider) {
    var $btn_box = $slider.siblings('.title').find('.btn-box');
    var $main_slider_text_box = $slider.find('.text-box');
    ...
    $main_slider_text_box.find('> .side-btns > div').click(function() {
        var index = $(this).index();
...
function SliderK__initAutoplay($slider) {
    ...
    if ( $slider.attr('data-play-stop-button') !== 'Y' ) {
        ...
        $btn_box.append(html);
        $main_slider_text_box.append(html);
        var $autoplay_btn_box = $slider.find('.autoplay-btn-box');
        
        if ( $autoplay_btn_box.length == 0 ) {
            $autoplay_btn_box = $slider.parent().find('.autoplay-btn-box');
        }

        $autoplay_btn_box.find('> .btn-start-play').click(function() {
            $slider.attr('data-autoplay-now-work', 'Y');

            $autoplay_btn_box.find('> .btn-start-play').hide();
            $autoplay_btn_box.find('> .btn-stop-play').show();
        });

        $autoplay_btn_box.find('> .btn-stop-play').click(function() {
            $slider.attr('data-autoplay-now-work', 'N');
            $autoplay_btn_box.find('> .btn-start-play').show();
            $autoplay_btn_box.find('> .btn-stop-play').hide();
        });
    }
